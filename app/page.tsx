...
const STORAGE_KEY = 'water_monitor_data';
const INITIAL_WATER_LEVEL = 124.6;
const TOTAL_HEIGHT = 150; // water_level + distance ≈ 150

function getStatus(level: number): WaterData['status'] {
    if (level >= 130) return 'danger';
    if (level >= 120) return 'warning';
    return 'normal';
}

/** Lấy mốc cập nhật gần nhất (xx:30) đã qua */
...
function createWaterData(waterLevel: number, updateTime?: Date): WaterData {
    const distance = Math.max(5, TOTAL_HEIGHT - waterLevel);
    const time = updateTime || getLastUpdateTime();
...
function saveToStorage(data: WaterData) {
    try {
...
/** Đọc từ localStorage */
function loadFromStorage(): WaterData | null {
    try {
...
/** Kiểm tra xem dữ liệu hiện tại còn thuộc mốc cập nhật này không */
function isSameUpdateSlot(saved: WaterData): boolean {
    const savedTime = new Date(saved.created_at);
    const currentSlot = getLastUpdateTime();

    return (
        savedTime.getFullYear() === currentSlot.getFullYear() &&
        savedTime.getMonth() === currentSlot.getMonth() &&
        savedTime.getDate() === currentSlot.getDate() &&
        savedTime.getHours() === currentSlot.getHours() &&
        savedTime.getMinutes() === 30
    );
}
...
export default function Home() {
    ...
    useEffect(() => {
        function updateWater() {
            ...
            // Nếu đã có dữ liệu của đúng mốc hiện tại → dùng lại
            if (saved && isSameUpdateSlot(saved)) {
                setData(saved);
                return;
            }

            // Chưa có hoặc đã qua mốc mới → tạo dữ liệu mới
            let newLevel = INITIAL_WATER_LEVEL;

            if (saved) {
                // Random nhẹ dựa trên giá trị cũ
                const levelChange = (Math.random() - 0.5) * 3;
                newLevel = Math.max(50, Math.min(145, saved.water_level_cm + levelChange));
            }

            const newData = createWaterData(newLevel, currentSlot);
        ...
        // Kiểm tra mỗi phút xem đã đến mốc :30 mới chưa
        const interval = setInterval(updateWater, 60 * 1000);
